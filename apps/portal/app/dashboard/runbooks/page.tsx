...
const SCENARIO_COLORS: Record<string, string> = {
  ...
  custom: "bg-surface-2 text-content-muted border-border",
};
...
const STATUS_COLORS: Record<string, string> = {
  ...
  pending: "bg-surface-2 text-content-muted border-border",
    ...
    <div className="p-7 bg-surface-2 min-h-screen">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-start justify-between mb-6">
          <div>
            <h1 className="text-xl font-bold text-content">DR Runbooks</h1>
            <p className="text-sm text-content-muted mt-1">Structured recovery playbooks for ransomware, site failover, and cloud outage scenarios</p>
          </div>
        ...
        <div className="flex gap-2 mb-5 flex-wrap">
          {scenarios.map(s => (
            <button key={s} onClick={() => setFilter(s)}
              className={`px-3 py-1.5 text-xs font-semibold rounded-full border capitalize transition-colors ${filter === s ? "bg-green-500 text-white border-green-500" : "bg-surface text-content-muted border-border hover:border-green-400"}`}>
              {s.replace("_", " ")}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          {filtered.map(r => (
            <div key={r.id} className="bg-surface border border-border rounded-xl p-5 shadow-sm">
              ...
              <h3 className="font-bold text-content text-base mb-2">{r.name}</h3>
              <div className="flex gap-1.5 mb-3 flex-wrap">
                {r.tags.map(t => <span key={t} className="text-[10px] bg-surface-2 text-content-muted px-2 py-0.5 rounded">#{t}</span>)}
              </div>
              <div className="text-xs text-content-muted mb-1">{r.step_count} steps &middot; {r.waves} waves &middot; Est. {Math.floor(r.est_mins/60)}h {r.est_mins%60}m</div>
              <div className="text-xs text-content-muted mb-3">RTO target: {r.rto_target_mins} min</div>
              {r.last_run ? (
                ...
                <div className="text-xs text-content-muted mb-3">Never executed</div>
              )}
              <div className="flex gap-2">
                <button className="flex-1 py-1.5 text-xs font-semibold text-content-muted border border-border rounded hover:bg-surface-2">View</button>
        ...
        <div className="bg-surface border border-border rounded-xl shadow-sm overflow-hidden">
          <div className="px-5 py-3.5 border-b border-border">
            <span className="text-sm font-bold text-content">Execution History</span>
          </div>
            ...
            <tbody>
              {MOCK_EXECUTIONS.map((e, i) => (
                <tr key={i} className="border-b border-border hover:bg-surface-2">
                  <td className="px-5 py-3 text-sm font-medium text-content">{e.runbook}</td>
                  <td className="px-5 py-3 text-sm text-content-muted">{e.trigger}</td>
                  <td className="px-5 py-3 text-xs text-content-muted">{e.started}</td>
                  <td className="px-5 py-3 text-sm text-content">{e.duration ? `${e.duration} min` : "-"}</td>
                  <td className="px-5 py-3 text-sm text-content-muted">{e.target} min</td>
