  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-surface rounded-xl shadow p-5 space-y-4">
        <h2 className="text-lg font-semibold text-content">Organization</h2>
        {orgLoading ? (
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-content-muted mb-1">Org ID</label>
              <p className="font-mono text-sm text-content bg-surface-2 border border-border rounded px-3 py-2">
                {org.id}
              </p>
            </div>
            <div>
              <label className="block text-sm text-content-muted mb-1">Org Name</label>
              <div className="flex items-center gap-3">
                <input
                  type="text"
                  value={orgName}
                  onChange={(e) => setOrgName(e.target.value)}
                  className="border border-border rounded-lg px-3 py-2 text-sm w-72 focus:outline-none focus:ring-2 focus:ring-veeam-green"
                />
                <button
      ...
      {/* Section 2: Notification Channels */}
      <div className="bg-surface rounded-xl shadow p-5 space-y-4">
        <h2 className="text-lg font-semibold text-content">Notification Channels</h2>
        {channelsLoading ? (
          <div className="flex items-center gap-2">
            <div className="animate-spin border-4 rounded-full w-5 h-5 border-green-500 border-t-transparent" />
            <span className="text-sm text-content-muted">Loading...</span>
          ...
          <p className="text-sm text-content-muted">No notification channels configured yet.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {channels.map((ch) => (
                  <tr key={ch.id} className="border-b hover:bg-surface-2">
                    <td className="py-2 pr-4 font-medium text-content">{ch.name}</td>
                    <td className="py-2 pr-4">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                          typeBadge[ch.type] ?? "bg-surface-2 text-content-muted"
                        }`}
                      >
                        {ch.type}
                      </span>
                    </td>
                    <td className="py-2 pr-4 text-content-muted text-xs truncate max-w-xs">
                      {ch.destination}
                    </td>
        ...
        <div className="border-t pt-4 space-y-3">
          <h3 className="text-sm font-semibold text-content">Add Channel</h3>
          <div className="flex flex-wrap gap-3 items-end">
            <div>
              ...
              <input
                ...
                className="border border-border rounded-lg px-3 py-2 text-sm w-40 focus:outline-none focus:ring-2 focus:ring-veeam-green"
              />
            </div>
            <div>
              ...
              <select
                ...
                className="border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-veeam-green"
              >
            ...
            <div>
              <label className="block text-xs text-content-muted mb-1">Destination</label>
              <input
                type="text"
                value={newDest}
                onChange={(e) => setNewDest(e.target.value)}
                placeholder={destinationPlaceholder(newType)}
                className="border border-border rounded-lg px-3 py-2 text-sm w-64 focus:outline-none focus:ring-2 focus:ring-veeam-green"
              />
            </div>
          </div>
          <div>
            <label className="block text-xs text-content-muted mb-1">Events</label>
            <div className="flex flex-wrap gap-4">
              {EVENT_OPTIONS.map((opt) => (
                <label key={opt.value} className="flex items-center gap-1.5 text-sm text-content cursor-pointer">
                  <input
                    type="checkbox"
      ...
      {/* Section 3: Default Targets */}
      <div className="bg-surface rounded-xl shadow p-5 space-y-4">
        <h2 className="text-lg font-semibold text-content">Default Targets</h2>
        ...
        {defaultsLoading ? (
          ...
          <div className="flex flex-wrap gap-4 items-end">
            <div>
              <label className="block text-sm text-content-muted mb-1">Default RTO (mins)</label>
              <input
                type="number"
                min={0}
                value={defaultRto}
            ...
            <div>
              <label className="block text-sm text-content-muted mb-1">Default RPO (mins)</label>
              <input
                type="number"
                min={0}
                value={defaultRpo}
