...
&nbsp;&nbsp;&nbsp;**[&#127760; Live Sample Demo](https://omarrao.github.io/r3vp/demo.html)**&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;**[&#128506; Capability Map](https://omarrao.github.io/r3vp/capability-map.html)**&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;**[&#128220; API Reference](https://omarrao.github.io/r3vp/api-spec/)**&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;**[&#127959; Architecture](docs/architecture.md)**&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;**[&#11015; Installing the Appliance](#installation)**
...
The AI insights module provides three capabilities without an external LLM dependency: RTO trend prediction using linear regression to flag workloads trending toward breach before it happens, statistical anomaly detection over recovery time series using z-score analysis, and a natural language query interface for common recovery posture questions. Workloads are ranked by a composite risk score across test recency, RTO proximity, and failure rate. The RTO prediction and anomaly detection run over each workload's real recorded test-run history (org-scoped), workload risk ranking is computed from live per-workload aggregates (latest RTO, failure rate, days since last test), the natural-language query answers from a live org-scoped context (workload counts, readiness score, active threats, recent failures, RTO breaches, provider pass rates), and the dashboard readiness score is computed server-side from live coverage, pass-rate, and RTO/RPO aggregates with a rolling 12-week trend.
...
**Strict tenant isolation on appliance ingest.** Every appliance-facing write is scoped to the authenticated appliance's organization. Test-run progress and results are accepted only for runs whose workload the calling appliance owns, and threat-scan ingest derives its organization from the mTLS-verified gateway identity rather than any request-body field, so a compromised or misconfigured appliance cannot read or falsify another tenant's data. List endpoints bound their result size, and authentication failures return generic messages with the detail logged server-side only.

**SSRF protection on outbound integrations.** Any URL you configure for an outbound integration (Slack/Teams/generic webhooks, SOAR platforms such as Splunk SOAR and XSOAR, and the ServiceNow, Jira, and Splunk HEC connectors) is validated before the platform ever requests it. Non-`http(s)` schemes are rejected at save time, along with any URL whose host is, or resolves to, a loopback, private (RFC1918), link-local, or otherwise non-public address, which blocks the cloud metadata endpoint (`169.254.169.254`) and internal-network pivots. Because a hostname can change what it resolves to, the resolved address is re-checked at dispatch time, immediately before the request is sent, and a request that fails the check is dropped and logged rather than made. Product infrastructure integrations that legitimately target your internal network (syslog SIEM, VeeamONE) are exempt.
